...
print("列表转化为字典--场景模拟结果: ", dictionary)  # 使用print()输出的是字典中的全部内容
# 在使用字典的时候很少直接输出它的内容, 一般情况是根据指定的键得到响应的结果
# 在Python中访问字典元素可以通过下标来实现,不过这个下标与列表.元组不一样,这里的下标指的是:dict[键]
# 我们想要获取"冷伊一"对应的值,
print("通过键获取值 如果指定的键不存在,程序就会报错： 例如获取'冷伊一'键对应的星座值是 : ",
      "\n   ", dictionary['冷伊一'])
# 通过if语句判断想要的键是否在字典中
print("通过if语句判断想要的键是否在字典中 例如想要获取'绮梦'对应的值! 如果没有就显示 我的字典里没有此人 ",
      "\n   ", dictionary["绮梦"] if "绮梦" in dictionary else "我的字典里没有此人")

# Python中提供了另外一种方法使用字典对象的get()方法获取指定键的值    dictionary.get(key,[default])
print("通过字典对象的get()方法获取键的值方法 1 使用字典对象的get()方法获取指定键的值 ", dictionary.get("香凝"))  # 使用字典对象的get()方法获取指定键的值
print("通过字典对象的get()方法获取键的值方法 2 指定的键值不存在就返回 None", dictionary.get("香"))  # 在使用get()方法时候,指定的键值不存在就返回"None"
print("通过字典对象的get('香', '查无此人') 方法获取键的值方法 3 指定键不存在时候的默认值", dictionary.get("香", "查无此人"))  # 添加了指定键不存在时候的默认值
...
print("\n 她的星座性格特点是: ", sign_dict.get(dictionary.get("香凝")))  # 输出性格特点


# keys方法用于获取所有键
dicts = {'绮梦': '水瓶座', '冷伊一': '射手座', '香凝': '双鱼座', '黛兰': '双鱼座'}
print("遍历字典方法 字典.items()  " + str(dicts.items()))
print("keys方法用于获取所有键 " + str(dicts.keys))
# values方法返回所有值
print("values方法返回所有值 " + str(dicts.values()))
