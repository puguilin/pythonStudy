...
#
# @Time    : 2023/2/21 17:20
# @Author  : pgl
# @File    : 01_爬取网站的图片.py.py
# @IDE     : PyCharm
...
def getUrlList(url):  # 获取图片链接
    ...
    # （类）做区分，在最后面添加了_
    nls = re.findall(r'src="(.*?)"', str(sp))  # 用正则表达式提取链接
    for i in nls:
        if 'https' in i:  # 因所有无效链接中均含有'https'字符串，因此直接剔除无效链接（对应第3条的分析）
            continue
        url_list.append(i)  # 在获取的链接中添加前缀，形成完整的有效链接
    return url_list
...
def fillPic(url, page):
    ...
    for p in range(len(pic_url)):
        # pic = getHtml(pic_url[p]) soup = BeautifulSoup(pic, 'html.parser') psoup = soup.find('div', class_="pic")
        # #class_="pic"作为锚，获取唯一div标签；注意，这里的网页源代码是class，但是python为了和class（类）做区分，在最后面添加了_ picUrl = re.findall(r'src="(
        # .*?)"', str(psoup))[0] #利用正则表达式获取详情图片链接，因为这里返回的是列表形式，所以取第一个元素（只有一个元素，就不用遍历的方式了）
        pic = requests.get(pic_url[p]).content  # 打开图片链接，并以二进制形式返回（图片，声音，视频等要以二进制形式打开）
        image_name = 'girls' + '第{}页'.format(page) + str(p + 1) + '.jpg'  # 给图片预定名字
...
def main():
    ...
    if not os.path.exists('./girls'):  # 如果不存在，创建文件目录
        os.mkdir('./girls/')
    page = 1
